...
class TournamentRegistrationController extends Controller
{
    ...
    public function index(Tournament $tournament, Request $request): View
    {
        ...
        $query = $tournament->registrations()
            ->with(['player.user', 'player.actualTeam.tournament', 'player.team', 'player.playerType', 'player.battingProfile', 'player.bowlingProfile', 'processedBy', 'actualTeam'])
    ...
    public function approvePendingChanges(Tournament $tournament, TournamentRegistration $registration): RedirectResponse
    {
        ...
        // Build human-readable summary before applying changes
        $fmt = fn ($field, $val) => match ($field) {
            'image_path' => 'Updated',
        ...
        foreach ($changes as $field => $newVal) {
            $changeSummary[] = [
                'label' => \App\Models\ProfileChangeLog::fieldLabel($field),
                'old' => $fmt($field, $player->{$field}),
                'new' => $fmt($field, $newVal),
            ];
        }
        ...
        if ($email && ! empty($changeSummary)) {
            try {
                ...
                \Illuminate\Support\Facades\Log::error('Failed to send profile changes approved email: ' . $e->getMessage());
            }
        }
