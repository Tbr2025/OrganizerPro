...
<head>
    ...
    <title>Profile change request rejected</title>
</head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: {{ $primaryColor }}; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
        ...
        <h1 style="color: #ffffff; margin: 0; font-size: 22px;">Profile Change Request Rejected</h1>
        <p style="color: rgba(255,255,255,0.85); margin: 10px 0 0 0;">{{ $tournamentName }}</p>
    </div>

    <div style="background: #f8f9fa; padding: 30px; border: 1px solid #e9ecef; border-top: none;">
        <p style="margin: 0 0 16px 0;">Hi <strong>{{ $playerName }}</strong>,</p>
        <p style="margin: 0 0 16px 0;">Your profile change request has been <strong style="color: #dc2626;">rejected</strong>. The following requested changes were not applied:</p>

        <div style="background: white; border-radius: 8px; padding: 6px 16px; margin-bottom: 20px; border-left: 4px solid #dc2626;">
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
                <tr style="text-align: left; color: #6c757d; font-size: 11px; text-transform: uppercase;">
                    ...
                    <th style="padding: 8px;">Requested Value</th>
                </tr>
                @foreach($changes as $c)
                <tr style="border-top: 1px solid #eee;">
                    <td style="padding: 10px 8px 10px 0; font-weight: 600; color: #495057;">{{ $c['label'] }}</td>
                    <td style="padding: 10px 8px; color: #6c757d;">{{ $c['value'] === '' ? '—' : $c['value'] }}</td>
                </tr>
                @endforeach
            </table>
        </div>

        <p style="margin: 0; color: #6c757d; font-size: 13px;">If you believe this was a mistake, please contact the tournament organizer.</p>
    </div>
