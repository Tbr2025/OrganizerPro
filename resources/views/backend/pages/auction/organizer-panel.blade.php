    ...
    <div class="p-4 mx-auto max-w-7xl md:p-6 lg:p-8">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Managing: <span
        ...
        <div x-data="organizerPanel()" x-init="init(
            {{ $auction->id }},
            '{{ $auction->status }}',
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-bold text-lg"
                            ...
                            x-text="auctionStatus.charAt(0).toUpperCase() + auctionStatus.slice(1)"></span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border">
                    ...
                    {{-- Tumbler Display --}}
                    <div
                        class="h-24 bg-gray-900 rounded-lg flex items-center justify-center text-center overflow-hidden relative">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border">
                    ...
                    <div x-show="!currentPlayer" x-cloak class="text-center py-10 text-gray-500">Waiting for next player...
                    </div>

                    <div x-show="currentPlayer" x-cloak class="flex flex-col items-center text-center">
                        <img :src="currentPlayer.player.image_path ? `/storage/${currentPlayer.player.image_path}` :
                            `https://ui-avatars.com/api/?name=${encodeURIComponent(currentPlayer.player.name)}`"
                            class="w-24 h-24 rounded-full object-cover mb-3 border-4 border-blue-500">
                        <h3 class="text-2xl font-bold" x-text="currentPlayer.player.name"></h3>
                        <p class="font-bold text-xl mt-2">Current Bid: <span class="text-green-500"
                                x-text="formatCurrency(currentBid)"></span></p>
                        ...
                        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 mt-4">
                            <div class="bg-blue-600 h-2.5 rounded-full timer-bar-inner" :style="`width: ${timerWidth}%`">
                            </div>
                        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border">
                <div class="p-5 border-b">
                    <h2 class="text-xl font-semibold">Live Bid History</h2>
                </div>
                        ...
                        <li class="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-md flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-4">
                                <span class="text-lg font-bold text-green-600 dark:text-green-400"
                                    x-text="formatCurrency(bid.amount)"></span>
                                <span class="text-xs text-gray-400"
                                    x-text="new Date(bid.created_at).toLocaleTimeString()"></span>
                            </div>
                        </li>
                    </template>
                    <li x-show="bidLog.length === 0" x-cloak class="text-center py-10 text-gray-500">Waiting for bids...
                    </li>
                </ul>
            </div>
        </div>
            ...
            return {
                ...
                init(auctionId, initialStatus, initialPlayers) {
                    ...
                    window.Echo.private(`auction.${this.auctionId}`)
                        .listen('.player-on-bid', (e) => this.handlePlayerOnBid(e))
                ...
                handlePlayerSold(event) {
                    ...
                    if (this.auctionStatus === 'running' && this.availablePlayers.length > 0) {
                ...
                startTumbler() {
                    ...
                    const shuffleInterval = setInterval(() => {
                        ...
                        if (shuffleCount >= maxShuffles) {
                            ...
                            this.tumblerText = this.availablePlayers[randomIndex].name;
                            this.statusText =
                                `Selected: ${this.availablePlayers[randomIndex].name}. Putting on bid...`;
                ...
                putPlayerOnBid() {
                    ...
                    this.sendCommand('player-on-bid', {
                        auction_player_id: this.selectedPlayerId
                    });
                },
                sellPlayer() {
                    if (this.currentPlayer) this.sendCommand(`sell-player`, {
                        auction_player_id: this.currentPlayer.id
                    });
                },
                passPlayer() {
                    if (this.currentPlayer) this.sendCommand(`pass-player`, {
                        auction_player_id: this.currentPlayer.id
                    });
                },
