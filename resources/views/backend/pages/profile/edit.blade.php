            ...
            <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
                ...
                <div class="p-5 space-y-6 border-t border-gray-100 dark:border-gray-800 sm:p-6">
                    ...
                    <form action="{{ route('profile.update') }}" method="POST" class="space-y-6">
                        @method('PUT')
                        @csrf
                        @php $isPlayerOnly = $user->hasRole('Player') && !$user->hasAnyRole(['Superadmin', 'Admin', 'Organizer']); @endphp
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div class="space-y-1">
                                <label for="name"
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Name') }}</label>
                                @if($isPlayerOnly)
                                    <p class="text-sm text-gray-900 dark:text-white py-2">{{ $user->name }}</p>
                            ...
                            <div class="space-y-1">
                                ...
                                @if($isPlayerOnly)
                                    <p class="text-sm text-gray-900 dark:text-white py-2">{{ $user->email }}</p>
                            ...
                            <x-inputs.password name="password" label="{{ __($isPlayerOnly ? 'New Password' : 'Password (Optional)') }}" />
                        ...
                        <div class="mt-6 flex justify-start gap-4">
                            ...
                            <a href="{{ $isPlayerOnly ? route('home') : route('admin.dashboard') }}"
