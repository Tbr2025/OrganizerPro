    ...
    <div class="p-4 mx-auto max-w-4xl md:p-6" x-data="{
        ...
        <div class="mt-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-md rounded-xl overflow-hidden">
            ...
            <div class="{{ $registration->type == 'player' ? 'p-8' : 'p-6' }} {{ $registration->type == 'team' ? 'bg-gradient-to-r from-purple-600 to-indigo-700' : 'bg-gradient-to-r from-blue-600 to-cyan-700' }}">
                <div class="flex {{ $registration->type == 'player' ? 'flex-col sm:flex-row' : '' }} justify-between items-start">
                    ...
                    @elseif($registration->status == 'cancelled')
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-400 text-gray-900">
                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6">
                                            ...
                                            <div x-data="{ editing: false, val: '{{ addslashes($value ?? '') }}', saving: false }" class="mt-1">
                                                ...
                                                <div x-show="editing" x-cloak class="flex items-center gap-1.5">
                                                    <input type="text" x-model="val" x-ref="editInput" @keydown.enter="$refs.saveBtn.click()" @keydown.escape="editing = false"
                                                           x-init="$watch('editing', v => { if (v) $nextTick(() => $refs.editInput.focus()) })"
                                                           class="flex-1 text-sm rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-2 py-1">
                                                    <button type="button" x-ref="saveBtn" :disabled="saving"
                                                            @click="saving = true; fetch('{{ route('admin.tournaments.registrations.update-field', [$tournament, $registration]) }}', {
                                                                method: 'PATCH', headers: {'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}'},
                                                                body: JSON.stringify({field: '{{ $key }}', value: val || null})
                                                            }).then(r => r.json()).then(d => { saving = false; editing = false; if(d.value !== undefined) val = d.value ?? ''; }).catch(() => { saving = false; })"
                                                            class="text-green-600 hover:text-green-700 text-xs px-1.5 py-1 rounded bg-green-50 dark:bg-green-900/30">
                                                        <i class="fas fa-check"></i>
                                                    </button>
                                                    <button type="button" @click="editing = false" class="text-gray-400 hover:text-gray-600 text-xs px-1.5 py-1">
                ...
                {{-- Icon Player Welcome Card Preview & Download (retained players) --}}
                @if($registration->isPlayerRegistration() && $registration->player && $registration->player->player_mode === 'retained')
                <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700"
                     x-data="{ loading: false, imageUrl: null, error: null }">
                    <h4 class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">Icon Player Welcome Card</h4>
                    <div class="flex flex-wrap items-center gap-3 mb-3">
                        <button type="button"
                                @click="loading = true; error = null;
                            ...
                            <span x-text="loading ? 'Generating...' : 'Preview Icon Player Welcome Card'"></span>
                        </button>
                        <a href="{{ route('admin.tournaments.registrations.retained-welcome-card.download', [$tournament, $registration]) }}"
                           class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg border border-indigo-300 dark:border-indigo-700 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                            Download Icon Player Welcome Card
                        </a>
                    </div>
                    <p x-show="error" x-text="error" class="text-sm text-red-500 mb-3" x-cloak></p>
                    <div x-show="imageUrl" x-cloak class="mt-3 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 inline-block">
                        <img :src="imageUrl" alt="Icon Player Welcome Card Preview" class="max-w-full max-h-[500px] object-contain">
                    </div>
                </div>
                @endif
                ...
                {{-- Actions --}}
                @if($registration->status == 'pending')
                    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                        <a href="{{ route('admin.tournaments.registrations.index', $tournament) }}"
                           class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-gray-700 bg-white dark:bg-gray-800 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            <form action="{{ route('admin.tournaments.registrations.reject', [$tournament, $registration]) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit"
                                    class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md shadow-sm hover:bg-red-700">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                    </svg>
                                    Reject Registration
                ...
                @else
                    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                        ...
                        <form action="{{ route('admin.tournaments.registrations.force-delete', [$tournament, $registration]) }}" method="POST" class="inline"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-red-800 border border-transparent rounded-md shadow-sm hover:bg-red-900">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
