                    ...
                    <select id="playerSelect" class="hidden">
                        ...
                        @foreach($players as $player)
                            ...
                            <option value="{{ $player->id }}"
                                    ...
                                    data-team="{{ $player->playing_team_name_ref ?: ($effectiveTeam?->name ?? $player->actualTeam?->name) }}"
                                    data-team-id="{{ $player->playing_team_name_ref ? '' : $effectiveTeamId }}"
                                    data-team-logo="{{ $player->playing_team_name_ref ? '' : ($effectiveTeam?->team_logo_url ?? $player->actualTeam?->team_logo_url ?? '') }}"
                                    data-playing-team-name="{{ $player->playing_team_name_ref ?: ($effectiveTeam?->name ?? $player->actualTeam?->name ?? '') }}"
                                    data-playing-team-logo="{{ $player->playing_team_name_ref ? '' : ($effectiveTeam?->team_logo_url ?? $player->actualTeam?->team_logo_url ?? '') }}"
    ...
    @php
        $playerDropdownData = $players->map(function($p) use ($tournamentTeamMap, $teams) {
            ...
            return [
                ...
                'team' => $p->playing_team_name_ref ?: ($effTeam?->name ?? $p->actualTeam?->name),
                'teamId' => $p->playing_team_name_ref ? '' : (string) $effTeamId,
                'teamLogo' => $p->playing_team_name_ref ? '' : ($effTeam?->team_logo_url ?? $p->actualTeam?->team_logo_url ?? ''),
                'playingTeamName' => $p->playing_team_name_ref ?: ($effTeam?->name ?? $p->actualTeam?->name ?? ''),
                'playingTeamLogo' => $p->playing_team_name_ref ? '' : ($effTeam?->team_logo_url ?? $p->actualTeam?->team_logo_url ?? ''),
