...
<div x-data="imageAdjustment('{{ $placeholder }}')" x-init="syncHidden()"
    ...
    {{--
        The Mirror / Flip toggles were here and are deliberately gone.

        Captain artwork is supplied as a pre-composed graphic with the team name,
        sponsor logo and badge baked into the PNG, so mirroring it reversed all of
        that lettering — the control could not do anything useful until the source
        images are bare player cut-outs. Swapping which side each team occupies is
        what was actually wanted; that lives on the match panel as "Swap sides".

        The render side still supports it (TemplateRenderService::overrideImageFlip),
        so restoring the buttons means re-adding markup that writes JSON
        {horizontal, vertical} into a hidden `flip_<placeholder>` input — getSelectedData()
        already collects any such input generically.
    --}}
</div>
