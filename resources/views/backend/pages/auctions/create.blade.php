            ...
            <form action="{{ route('admin.auctions.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="base_price" id="base_price"
                        value="{{ old('base_price', 1.0) }}" class="mt-1 block w-full rounded-lg border-gray-300">
                    @error('base_price')
                        <span class="text-sm text-red-600">{{ $message }}</span>
                    @enderror
                </div>
                ...
                {{-- Max Bid per Player --}}
                <div>
                    <label for="max_bid_per_player" class="block text-sm font-medium">Max Bid per Player (in
                        Millions)</label>
                    <input type="number" step="0.01" name="max_bid_per_player" id="max_bid_per_player"
                        value="{{ old('max_bid_per_player', 6.0) }}" class="mt-1 block w-full rounded-lg border-gray-300">
                    @error('max_bid_per_player')
                        <span class="text-sm text-red-600">{{ $message }}</span>
                    @enderror
                </div>
                ...
                {{-- Max Budget per Team --}}
                <div>
                    <label for="max_budget_per_team" class="block text-sm font-medium">Max Budget per Team (in
                        Millions)</label>
