                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                        @foreach ($auctions as $auction)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $loop->iteration + ($auctions->currentPage() - 1) * $auctions->perPage() }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $auction->name }}</td>
...
<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
    {{ $auction->end_at ? $auction->end_at->format('Y-m-d') : '-' }}
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2">
                                    ...
                                    <a href="{{ route('admin.auctions.edit', $auction) }}" class="text-yellow-600 hover:text-yellow-900">Edit</a>
                                    <form action="{{ route('admin.auctions.destroy', $auction) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this auction?');">
                                        @csrf
                                        @method('DELETE')
