    ...
    <div class="p-4 mx-auto max-w-7xl md:p-6 lg:p-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700"
            x-data="{
                ...
                search: '',
                searchInPool: '', // for players in pool
                ...
                <div class.blade.php="p-4 border-b border-gray-200 dark:border-gray-700">
                    <nav class="flex flex-wrap space-x-4" aria-label="Tabs">
                        <a href="#" @click.prevent="step = 1"
                            ...
                            class="px-3 py-2 font-medium text-sm rounded-md border-b-2">1. Details</a>
                        <a href="#" @click.prevent="step = 2"
                            ...
                            class="px-3 py-2 font-medium text-sm rounded-md border-b-2">2. Financials</a>
                        <a href="#" @click.prevent="step = 3"
                            ...
                            class="px-3 py-2 font-medium text-sm rounded-md border-b-2">3. Bid Rules</a>
                ...
                <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="name" id="name"
                                    value="{{ old('name', $auction->name) }}" class="form-control" required>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="base_price" id="base_price"
                                    value="{{ old('base_price', $auction->base_price) }}" class="form-control" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="step === 3" x-transition.opacity x-cloak>
                        <div>
                            ...
                            <div class="space-y-3">
                                <template x-for="(rule, index) in rules" :key="index">
                                    <div class="flex items-center gap-2 bg-gray-50 dark:bg-gray-900/50 p-3 rounded-md">
                                        <span class="text-gray-500">From</span><input type="number"
                                            :name="`bid_rules[${index}][from]`" x-model="rule.from"
                                            class="form-control w-1/4">
                                        <span class="text-gray-500">To</span><input type="number"
                                            :name="`bid_rules[${index}][to]`" x-model="rule.to" class="form-control w-1/4">
                                        <span class="text-gray-500">Increment</span><input type="number"
                                            :name="`bid_rules[${index}][increment]`" x-model="rule.increment"
                                            class="form-control w-1/4">
                            ...
                            <button type="button"
                                @click="rules.push({ from: rules.length ? rules[rules.length-1].to : 0, to: '', increment: '' })"
                                class="btn btn-secondary mt-4">+ Add Rule</button>
                        </div>
                    </div>
                    ...
                    <div x-show="step === 4" x-transition.opacity x-cloak>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div class="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg border">
                                <h4 class="font-semibold mb-2">Players in Pool (<span x-text="inAuction.length"></span>)
                                </h4>
                                ...
                                <div class="space-y-2 max-h-96 overflow-y-auto">
                                    <template x-for="player in inAuctionFiltered" :key="player.id">
                                        <div
                                            class="flex justify-between items-center p-2 bg-white dark:bg-gray-800 rounded shadow-sm">
                                            <span x-text="player.name"></span>
                                            <div class="flex items-center gap-2">
                                                <input type="number" :name="`player_base_prices[${player.id}]`"
                                                    x-model="player.base_price" class="form-control form-control-sm w-28"
                            ...
                            <div class="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg border">
                                ...
                                <div class="space-y-2 max-h-96 overflow-y-auto">
                                    ...
                                    <div x-show="filteredAvailable.length === 0" class="text-center text-gray-500 py-4">No
                                        matching players found.</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <button type="button" x-show="step < 4" @click="step++" class="btn btn-primary"
                            x-cloak>Next</button>
                        <button type="submit" x-show="step === 4" class="btn btn-success" x-cloak>Update Auction &
                            Pool</button>
                    </div>
                </div>
            </form>
