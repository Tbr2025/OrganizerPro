    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-100 dark:border-gray-700">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ format_millions($totalBudget) }}</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-100 dark:border-gray-700">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ format_millions($totalBudget - $totalSpent) }}</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-100 dark:border-gray-700">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ format_millions($totalSpent) }}</p>
...
@php
    $playersJsonData = $teamPlayers->map(function($player) {
        return [
            ...
            // How they were acquired. player_mode reads `retained` for a purchase too, so it
            // cannot label this — see SquadAcquisitionService.
...
<script>
    ...
    function openViewPlayerModal(playerId) {
        ...
        html += `<div><h4 class="text-xl font-bold text-gray-900 dark:text-white">${player.name}</h4>
            ...
            ${player.acquisition_label ? `<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-gradient-to-r ${player.acquisition === 'auction' ? 'from-emerald-500 to-teal-600' : 'from-purple-500 to-violet-600'} text-white"><svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd"/></svg>${player.acquisition_label}${player.acquisition_price_label ? ' · ' + player.acquisition_price_label : ''}</span>` : ''}
            </div>
        </div></div>`;
        const fields = [
