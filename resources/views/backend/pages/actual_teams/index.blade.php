...
@section('admin-content')
    <div class="p-4 mx-auto max-w-7xl md:p-6 lg:p-8">
        ...
        <div class="space-y-6">
                ...
                <form action="{{ route('admin.actual-teams.index') }}" method="GET">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        @if (auth()->user()->hasRole('Superadmin'))
                            <div>
                                <label for="organization_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Filter by Organization</label>
                        ...
                        {{-- Tournament Filter --}}
                        <div>
                            <label for="tournament_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Filter by Tournament</label>
                        ...
                        <div class="flex items-end space-x-2 pt-2">
                            ...
                            <a href="{{ route('admin.actual-teams.index') }}" class="btn btn-secondary w-full sm:w-auto">Reset</a>
                        </div>
                    </div>
                </form>
            </div>
            ...
            {{-- Teams Table --}}
            <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] overflow-hidden">
                            ...
                            @forelse ($actualTeams as $team)
                                <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/40">
                                    <td class="px-4 py-2">
                                        ...
                                        @else
                                            <div class="h-10 w-10 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center text-xs text-gray-500">
                                                No Logo
                                            </div>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-end space-x-2">
                                            @can('actual-team.view')
                                                <a href="{{ route('admin.actual-teams.show', $team) }}" title="View" class="p-2 text-gray-500 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700">
                                                     <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                                </a>
                                            @endcan
                                            @can('actual-team.edit')
                                                 <a href="{{ route('admin.actual-teams.edit', $team) }}" title="Edit" class="p-2 text-blue-500 rounded-full hover:bg-blue-100 dark:hover:bg-blue-900/50">
                                            ...
                                            @can('actual-team.delete')
                                                <form method="POST" action="{{ route('admin.actual-teams.destroy', $team) }}" onsubmit="return confirm('Are you sure you want to delete this team?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" title="Delete" class="p-2 text-red-500 rounded-full hover:bg-red-100 dark:hover:bg-red-900/50">
