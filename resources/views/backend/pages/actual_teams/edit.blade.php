...
<div class="p-4 mx-auto max-w-7xl md:p-6 lg:p-8">
    ...
    <form id="team-form" action="{{ route('admin.actual-teams.update', $actualTeam) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="space-y-8">
                    ...
                    <div class="p-5 space-y-6">
                        ...
                        <div x-data="{
                            ...
                            handleFileChange(event) {
                                ...
                                if (file && file.type.startsWith('image/')) { this.previewUrl = URL.createObjectURL(file); } 
                                else { this.previewUrl = '{{ $actualTeam->team_logo ? Storage::url($actualTeam->team_logo) : '' }}'; }
                            }
                        }">
                            ...
                            <div class="mt-2 flex items-center gap-4">
                                ...
                                <label for="team_logo" class="cursor-pointer bg-white dark:bg-gray-700 py-2 px-3 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <span>Change Logo</span>
                                    <input id="team_logo" name="team_logo" type="file" class="sr-only" @change="handleFileChange">
                                </label>
                            </div>
                            @error('team_logo') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                    ...
                    <div id="current-members-container" class="p-5 space-y-3 max-h-[600px] overflow-y-auto">
                        {{-- **FIXED**: Roster loop now includes a role dropdown --}}
                        @forelse($currentMembers as $member)
                            <div id="current-member-{{ $member->id }}" class="flex items-center p-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                ...
                                {{-- Role Dropdown for Existing Members --}}
                                <div class="w-40 mr-2">
                                    <select name="user_roles[{{ $member->id }}]" class="form-control form-control-sm">
                                        @foreach ($roles as $role)
                                            <option value="{{ $role->name }}" {{ ($member->pivot->role ?? '') == $role->name ? 'selected' : '' }}>
                                                {{ ucfirst($role->name) }}
                                            </option>
                                        @endforeach
                                    </select>
            ...
            {{-- RIGHT COLUMN --}}
            <div>
                <!-- Available Players Card (unchanged) -->
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            userSearch.addEventListener('input', function() {
                ...
                availableContainer.querySelectorAll('[data-user-id]').forEach(row => {
                    ...
                    row.style.display = name.includes(searchValue) || email.includes(searchValue) ?
                        'flex' : 'none';
                });
            });
