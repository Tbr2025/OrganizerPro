...
## Team Manager UX — Simplified Dashboard

Team managers get a simplified UI compared to admins/organizers. Key design decisions:

### What Team Managers Should NOT See
- **Verification percentage / completion circles** — hidden from players list, player profile, and squad views
- **Field verification indicators** (green/orange borders, checkmark icons) — hidden on shared `players/show.blade.php`
- **Sidebar minification** — sidebar never collapses to 80px icon-only mode on desktop

### What Team Managers Get Differently
- **Sidebar submenus always expanded** — My Team, Auctions, Matches all open by default (no collapsed state)
- **"Menu" text label** on mobile hamburger button (less tech-savvy users)
- **Simplified sidebar menu** — only My Team, Auctions, Matches, Logout (no admin routes)
- **Email field hidden** on player profile view

### How Team Manager Detection Works
```php
$isTeamManagerLayout = auth()->user()?->hasAnyRole(['Team Manager', 'Team Owner'])
    && !auth()->user()?->hasAnyRole(['Superadmin', 'Admin', 'Organizer']);
```
- Set in `app.blade.php` as PHP variable `$isTeamManagerLayout`
- Passed to Alpine.js as `teamManagerLayout` boolean for sidebar behavior
- Used in `AdminMenuService::shouldExpandSubmenu()` to always expand submenus
- Used in `players/show.blade.php` as `$isTeamManagerView` for hiding verification UI

### Key Code Locations
| What | Where |
|------|-------|
| Team manager layout flag | `resources/views/backend/layouts/app.blade.php` — `$isTeamManagerLayout` |
| Sidebar never-minify logic | `resources/views/backend/layouts/partials/sidebar-logo.blade.php` — `teamManagerLayout` |
| Menu text label on mobile | `resources/views/backend/layouts/partials/header.blade.php` — hamburger button |
| Always-expand submenus | `app/Services/MenuService/AdminMenuService.php` — `shouldExpandSubmenu()` |
| Simplified menu definition | `app/Services/MenuService/AdminMenuService.php` — `getMenu()` (line ~167) |
| Hidden verification UI | `resources/views/backend/pages/players/show.blade.php` — `$isTeamManagerView` |
| Removed verified column | `resources/views/backend/pages/team-manager/players.blade.php` |
| Deduplicated player queries | `app/Http/Controllers/Backend/TeamManagerController.php` — `groupBy('players.id')` |

### Rules for Future Development
- Always check `$isTeamManagerView` or `$isTeamManagerLayout` before showing admin-only UI in shared views
- Team managers must never see verification status, approval controls, or admin-level data
- Player queries for team managers must include `groupBy('players.id')` to prevent duplicates from the `player_actual_team_tournament` pivot table
